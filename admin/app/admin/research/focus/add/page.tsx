...
const ICON_OPTIONS = [
  "FlaskConical", "Globe2", "Lightbulb", "Users",
  "Building2", "Award", "Microscope", "BookOpen",
];

export default function AddFocusPage() {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState<ResearchFocus>({
    icon: ICON_OPTIONS[0], title: "", desc: "",
    color: COLOR_OPTIONS[0].value, count: "",
  });
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      ...
      const updated = { ...current, focusAreas: [...current.focusAreas, form] };
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold" style={{ color: "var(--text)" }}>Add Focus Area</h1>
              <p className="text-xs mt-0.5" style={{ color: "var(--text-faint)" }}>Add a new research focus area</p>
            </div>
          </div>
        ...
        <div className="admin-card rounded-2xl p-6 space-y-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea name="desc" value={form.desc} onChange={handleChange} rows={3}
              placeholder="Brief description of this research focus area…"
          ...
          <div className="flex items-center gap-3 p-3 rounded-xl"
            ...
            <div>
              <p className="text-sm font-bold" style={{ color: "var(--text)" }}>{form.title || "Focus Area Title"}</p>
              <p className="text-xs" style={{ color: "var(--text-faint)" }}>{form.count || "0 Projects"}</p>
            </div>
          </div>
        </div>
