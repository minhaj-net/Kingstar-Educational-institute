...
import { ChevronLeft, Loader, Building2, Plus, X } from "lucide-react";
...
export default function EditULPage() {
  const { id } = useParams() as { id: string };
  const router = useRouter();
  const [form, setForm] = useState<ULItem|null>(null);
  const [notFound, setNotFound] = useState(false);
  const [saving, setSaving] = useState(false);
  const [errors, setErrors] = useState<Record<string,string>>({});

  useEffect(() => {
    const load = (data: ULItem[]) => {
    ...
    fetch("/university-life.json")
      .then(r=>r.json())
      .then((data: ULItem[]) => {
        sessionStorage.setItem("admin_university_life", JSON.stringify(data));
        load(data);
      })
  ...
  function addToList(field:"highlights"|"gallery", v:string) {
    setForm(p=>p ? {...p,[field]:[...(p[field] as string[]),v]} : p);
  }
  function removeFromList(field:"highlights"|"gallery", v:string) {
    setForm(p=>p ? {...p,[field]:(p[field] as string[]).filter(x=>x!==v)} : p);
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(()=>{
      ...
      const updated = all.map(item => item.id === id ? form : item);
  ...
  if (notFound) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      <p className="text-lg font-semibold" style={{ color:"var(--text)" }}>Item not found: {id}</p>
      <Link href="/admin/university-life"
        className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white"
        style={{ backgroundColor:"#4caf50" }}>← Back to University Life</Link>
    </div>
  );
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold" style={{ color:"var(--text)" }}>Edit University Life Item</h1>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 space-y-5">
            ...
            <FormSection title="Content">
              <Field label="Description" name="description" value={form.description} onChange={handleChange} multiline />
            ...
            <FormSection title="Cover Image">
              ...
              {imagePreview ? (
                ...
                <div className="flex flex-col items-center justify-center rounded-xl py-8"
                  ...
                  <Building2 className="w-8 h-8" style={{ color:"var(--text-faint)" }} />
