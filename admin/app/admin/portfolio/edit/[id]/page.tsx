...
import { ChevronLeft, Plus, X, Briefcase, Loader } from "lucide-react";
...
export default function EditPortfolioPage() {
  ...
  useEffect(() => {
    ...
    const loadItem = (data: PortfolioItem[]) => {
      ...
      setCategories(Array.isArray(found.categories) ? found.categories : []);
      setLoaded(true);
    };

    const saved = sessionStorage.getItem("admin_portfolio");
    if (saved) {
      loadItem(JSON.parse(saved));
    } else {
      fetch("/portfolio.json")
        .then((r) => r.json())
        .then((data: PortfolioItem[]) => {
          sessionStorage.setItem("admin_portfolio", JSON.stringify(data));
          loadItem(data);
        })
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      const saved = sessionStorage.getItem("admin_portfolio");
      const all: PortfolioItem[] = saved ? JSON.parse(saved) : [];
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        <Briefcase className="w-12 h-12" style={{ color: "var(--text-faint)" }} />
        <p className="text-lg font-semibold" style={{ color: "var(--text)" }}>
          Portfolio item #{itemId} not found
        </p>
        <Link
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "var(--text-faint)" }}>
                Editing:{" "}
                <span style={{ color: "#4caf50" }}>#{itemId}</span> —{" "}
                {form.title}
              </p>
            </div>
          </div>
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            <FormSection title="Basic Information">
              ...
              <p className="text-xs -mt-2" style={{ color: "var(--text-faint)" }}>
                Edit the slug manually if needed. It should be URL-friendly.
              </p>
            </FormSection>
