...
const CAT = ["Academics","Research","Events","Technology","Athletics","Campus Life","Alumni","Admissions"];
const TAGS = ["HOT","RESEARCH","EVENT","ARTICLE","UPDATES","STUDENT","ADMISSION","ALUMNI"];
...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed.find(b => String(b.id) === String(id))) { load(parsed); return; }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      ...
      if (!updated.find(b => String(b.id) === String(id))) updated.push(form);
  ...
  if (notFound) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      <p className="text-lg font-semibold" style={{ color:"var(--text)" }}>Blog post not found</p>
      <Link href="/admin/blogs" className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white"
        style={{ backgroundColor:"#4caf50" }}>← Back to Blogs</Link>
    </div>
  );
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="admin-card rounded-2xl p-5 space-y-3">
              ...
              {[["Author",form.author],["Category",form.category],["Tag",form.tag],["Date",form.date],["Read Time",form.readTime]]
                .map(([l,v])=>(
