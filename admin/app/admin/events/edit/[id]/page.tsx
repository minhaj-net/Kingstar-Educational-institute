...
export default function EditEventPage() {
  ...
  const [form, setForm] = useState<KUEvent | null>(null);
  const [notFound, setNotFound] = useState(false);
  const [saving, setSaving] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});

  useEffect(() => {
    const load = (data: KUEvent[]) => {
    ...
    fetch("/events.json").then((r) => r.json()).then((d: KUEvent[]) => {
      sessionStorage.setItem("admin_events", JSON.stringify(d)); load(d);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      ...
      sessionStorage.setItem("admin_events", JSON.stringify(all.map((ev) => String(ev.id) === String(id) ? form : ev)));
      setSaving(false);
  ...
  if (notFound) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      <p style={{ color: "var(--text)" }}>Event not found.</p>
      <Link href="/admin/events" className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white"
        style={{ backgroundColor: "#4caf50" }}>← Back to Events</Link>
    </div>
  );
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold" style={{ color: "var(--text)" }}>Edit Event</h1>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 admin-card rounded-2xl p-6 space-y-5">
            ...
            {/* Time & Location */}
            {(["time", "location"] as const).map((field) => (
              <div key={field} className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold capitalize" style={{ color: "var(--text-muted)" }}>{field}</label>
            ...
            {/* Image */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold" style={{ color: "var(--text-muted)" }}>Image URL</label>
          ...
          <div className="space-y-5">
            <div className="admin-card rounded-2xl p-5 space-y-3">
              ...
              {[
                { label: "Date",     value: `${form.day} ${form.month}` },
